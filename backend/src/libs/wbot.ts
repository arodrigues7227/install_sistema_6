    ...
    try {
      (async () => {
          ...
          if (messages && messages.length > 0) {
            for await (const [, msg] of Object.entries(messages)) {
              ...
              if (isValidMsg(msg) && dateOldLimit < timestampMsg && dateRecentLimit > timestampMsg) {
                ...
                const isStatusMessage = remoteJid?.includes("status@broadcast");
                
                // Pular mensagens de status
                if (isStatusMessage) {
                  continue;
                }
                
                // Processar mensagens de conversas privadas (TODAS as mensagens da conversa)
                if (!isGroupMessage) {
                  addLogs({
                    fileName: `preparingImportMessagesWppId${whatsapp.id}.txt`, 
                    text: `Adicionando mensagem para processamento:
        CONVERSA PRIVADA >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        Data e hora da mensagem: ${moment(timestampMsg).format("DD/MM/YYYY HH:mm:ss")}
        Contato da conversa: ${remoteJid}
        Remetente: ${msg.key?.fromMe ? 'EU' : 'CONTATO'}
        ...
        `});
                ...
                // Processar mensagens de grupos (se habilitado)
                else if (whatsapp?.importOldMessagesGroups) {
                  addLogs({
                    fileName: `preparingImportMessagesWppId${whatsapp.id}.txt`, 
                    text: `Adicionando mensagem para processamento:
        MENSAGEM DE GRUPO >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        `});
          ...
          // Adicionar mensagens ao array de dados
          if (!dataMessages?.[whatsappId]) {
            dataMessages[whatsappId] = [];
          }
          
          // Usar concat para evitar duplicatas e manter ordem
          dataMessages[whatsappId] = dataMessages[whatsappId].concat(filteredDateMessages);
          
          // Log estatísticas
          addLogs({
            fileName: `preparingImportMessagesWppId${whatsapp.id}.txt`, 
            text: `ESTATÍSTICAS DE IMPORTAÇÃO:
        Total de mensagens processadas: ${messages.length}
        Mensagens válidas no período: ${filteredDateMessages.length}
        Total acumulado para importação: ${dataMessages[whatsappId].length}
        Período: ${moment(dateOldLimit).format("DD/MM/YYYY")} até ${moment(dateRecentLimit).format("DD/MM/YYYY")}
