...
const UserModal = ({ open, onClose, userId }) => {
  ...
  const handleSaveUser = async (values) => {
    ...
    const userData = {
      ...
      canDeleteMessage: values.canDeleteMessage,
      showCampaign: values.showCampaign,
    };
    ...
    try {
      if (userId) {
        ...
        if (!user?.profileImage && avatar) uploadAvatar(user);
      }
                          ...
                          <Grid container spacing={1}>
                            <Grid item xs={12} md={6} xl={6}>
                              ...
                              >
                                <>
                                  <InputLabel>
                                    {i18n.t("userModal.form.canDeleteMessage")}
                                  </InputLabel>

                                  <Field
                                    as={Select}
                                    label={i18n.t(
                                      "userModal.form.canDeleteMessage"
                                    )}
